...
在本模型下运行方法:
1. ./finetune.sh
   该程序生成一个output目录，这是一个微调了的模型目录。
2. ./infer.sh
   该程序会利用模型目录中的模型，应用于测试集在output_dir变量说的目录下的一堆文件夹，即debug1,debug2,...,debug108。每个文件夹对应测试集中的一个音频文件。每个音频文件就是一句话，把它转录出来（经过模型转录或识别后与原文有一定区别）。
3. ./infer-paraformer.sh
   该程序不使用微调，而是直接使用模型"iic/speech_paraformer-large_asr_nat-zh-cn-16k-common-vocab8404-pytorch"进行推理。
4. ./tongjiText.sh
   将/tmp/output/test目录下的每个debug目录下的文件合并为一个文件textTotal.txt和tokenTotal.txt，前者应该相当于测试集，后者就是转录集合。后者为我们需要的，可以用来评估正确率。

下面为paraformer的README_zh.md中的原文内容。
<div align="center">  
<h4>
 <a href="#模型推理"> 模型推理 </a>   
